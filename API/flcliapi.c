...
static void flcliapi_agent (void *args,zsock_t *sock, void *pipe);
...
flcliapi_new (void)
{
    ...
    self->frontendsock = zsock_new_dealer("@inproc://#1");
...
flcliapi_destroy (flcliapi_t **self_p)
{
    ...
    if (*self_p) {
        ...
        zsock_destroy(&self->frontendsock);
        free (self);
        *self_p = NULL;
    }
}
...
agent_t *
agent_new (zsock_t *zsock, void *pipe)
{
...
agent_control_message (agent_t *self)
{
    ...
    if (streq (command, "CONNECT")) {
        ...
        assert (rc == 0);
        server_t *server = server_new (endpoint);zmsg_pushstr(request,"ciao");
...
static void
flcliapi_agent (void *args, zsock_t *zsock, void *pipe)
{
    agent_t *self = agent_new (zsock, pipe);
    ...
    while (!zctx_interrupted) {
        ...
        &&  tickless > self->expires)
            tickless = self->expires;
        server_tickless("hello", zhash_next(self->servers),&tickless);
        ...
        server_ping("hello",zhash_next(self->servers), self->router);
    }
    agent_destroy (&self);
}
